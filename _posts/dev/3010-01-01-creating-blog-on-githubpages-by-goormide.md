...
이번엔 아래처럼 이메일을 인증하라는 화면이 나온다. 일단은 가입은 완료가 되었기에 더 이상 진행할 필요는 없다.
...
"Choose a theme" 버튼만 눌러준다. 테마를 고르는 것만으로, 깃허브 페이지 운영에 필요한, 정적 홈페이지 제너레이터 툴인 [Jekyll](http://jekyllrb-ko.github.io/) 설치와 이런저런 초기 설정을 자동으로 해준다.
