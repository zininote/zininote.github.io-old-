...
위 코드는 `n` 이라는 숫자가 주어졌을 때, 그 이하의 수 중에서 소수를 찾아내는 방식이다. 개인적으로 `n` 이라는 숫자에 구애받지 않고 2 부터 시작해서 계속 소수를 구하는 방법은 없을까 궁금해했다. 이런 코드를 구글링으로 찾아서 [별도 포스팅](https://zininote.github.io/posts/prime-number-generator)에 정리하였으니 궁금하신 독자들은 한번 들러주길 바란다.
