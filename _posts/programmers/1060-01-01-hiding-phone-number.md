...
참고로, Javascript 와 Python 은 정규식 사용 방식이 약간 다르다. Javascript 는 정규식 표현식(/.../) 이 별도로 있으며, 문자열의 메소드 형태로 관련 함수를 제공하지만, Python 은 정규식을 사용하기 위한 별도의 개체 re 를 두고는, 이 개체에서 관련 함수를 제공하고 있다.